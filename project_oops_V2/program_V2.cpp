...
class result : protected account{
    ...
    int input(int n){
        cin.ignore();
        print_with_delay("\t\t\tMarks in subject :-\n");
        switch(n){
            case 1:
    ...
    void initialize(){
        ...
        while(getline(o,line)){
            if(line == "Examination = " + session){
                found = true;
                int count=0;
                string sub,tmp;
                int mark;
                while(getline(o,line)){
                    if(line == "Marks in all subjects :-"){
                        while(o>>sub>>tmp>>mark || count<5){
                            data[sub] = mark;
                            count++;
                        }
                    }
        ...
        if(!found){
            ...
            f<<"\n\nExamination = "<<session<<endl;
            f<<"Student = "<<user_name<<endl;
            f<<"Marks in all subjects :-"<<endl;
            f<<"Math = 0"<<endl;
            f<<"Science = 0"<<endl;
            f<<"Computer = 0"<<endl;
            f<<"English = 0"<<endl;
            f<<"Social = 0"<<endl;
            f<<"----------------------"<<endl;
            f<<"Total marks = 0"<<endl;
            f<<"Total Percentage = 0%"<<endl;
            f<<"Grade = FAIL"<<endl;
            f<<"\n\n";
            f.close();
            initialize();
    ...
    void update(){
        ...
        filename = "results/" + filename;
        old.open(filename);
        new_file.open(filename+"111");
        string old_line;
        bool found = false;
        while(getline(old,old_line)){
            if(old_line == "Examination = " + session && !found){
                new_file<<old_line<<endl;
                new_file<<"Student = "<<user_name<<endl;
                while(getline(old,old_line)){
                    if(old_line == "Marks in all subjects :-"){
                        new_file<<old_line<<endl;
                        new_file<<"Math = "<<math<<endl;getline(old,old_line);
                        new_file<<"Science = "<<science<<endl;getline(old,old_line);
                        new_file<<"Computer = "<<computer<<endl;getline(old,old_line);
                        new_file<<"English = "<<english<<endl;getline(old,old_line);
                        new_file<<"Social = "<<social<<endl;getline(old,old_line);
                        new_file<<"----------------------\n";getline(old,old_line);
                        new_file<<"Total marks = "<<get_total()<<endl;getline(old,old_line);
                        new_file<<"Total Percentage = "<<get_percent()<<"%"<<endl;getline(old,old_line);
                        if(get_percent()>90){
                            new_file<<"Grade = A+"<<endl;getline(old,old_line);
                        }
                        else if(get_percent()>80){
                            new_file<<"Grade = A"<<endl;getline(old,old_line);
                        }
                        else if(get_percent()>70){
                            new_file<<"Grade = B+"<<endl;getline(old,old_line);
                        }
                        else if(get_percent()>60){
                            new_file<<"Grade = B"<<endl;getline(old,old_line);
                        }
                        else if(get_percent()>50){
                            new_file<<"Grade = c+"<<endl;getline(old,old_line);
                        }
                        else if(get_percent()>40){
                            new_file<<"Grade = PASS"<<endl;getline(old,old_line);
                        }
                        else{
                            new_file<<"Grade = FAIL"<<endl;getline(old,old_line);
                        }
                        new_file<<"\n\n";getline(old,old_line);getline(old,old_line);
                        found = true;
                        break;
                    }
                }
        ...
        rename((filename+"111").c_str(),filename.c_str());
    }
    void all_students(account &a){
        for (const auto& entry : filesystem::directory_iterator("results/")) {
            if (entry.is_regular_file()) {
                user_name = entry.path().filename().stem().string();
                cout<<"\n\n";
                cout<<"\t\t\t"<<user_name<<":"<<endl;
                write(a,true);
            }
    ...
    void write(account &a,bool option=false){
        if(!option){
            print_with_delay("\t\t\tExamination = ",40,false);
            getline(cin,session);
            aaa:
            print_with_delay("\t\t\tStudents Username [write \"all\" for all students]= ",40,false);
            cin>>user_name;
            if(user_name == "all"){
                all_students(a);
                return;
        ...
        old.open(filename);
        string subject;
        ifstream d("users/"+a.return_username()+".txt");
        string line;
        while(getline(d,line)){
            //check teacher's subject
            if(line == "subject = math"){
                subject = "math";
                break;
            }
            else if(line == "Subject = science"){
                subject = "science";
                break;
            }
            else if(line == "Subject = computer"){
                subject = "computer";
                break;
            }
            else if(line == "Subject = english"){
                subject = "english";
                break;
            }
            else if(line == "Subject = social"){
                subject = "social";
                break;
            }
            else{
        ...
        while(getline(old,tmp_line)){
            if(tmp_line == "Examination = " + session){
                ...
                int line_no = 0;
                while(line_no<11){
                    getline(old,tmp_line);
                    line_no++;
                    if((subject == "math" || subject == "incharge") && tmp_line.find("Math")!=string::npos){
                        math = input(1);
                    }
                    if((subject == "science" || subject == "incharge") && tmp_line.find("Science")!=string::npos){
                        science = input(2);
                    }
                    if((subject == "computer" || subject == "incharge") && tmp_line.find("Computer")!=string::npos){
                        computer = input(3);
                    }
                    if((subject == "english" || subject == "incharge") && tmp_line.find("English")!=string::npos){
                        english = input(4);
                    }
                    if((subject == "social" || subject == "incharge") && tmp_line.find("Social")!=string::npos){
                        social = input(5);
                    }
                }
            }
...
void fill_details(account &a){
    ofstream data;
    string name,address,phone,DOB,position,DOJ,subject;
    int roll;
